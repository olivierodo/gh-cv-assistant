...
describe('Webhooks # push',  () => {
  ...
  describe('Retrieve file to compile', () => {
    ...
    test('Should create an issue when it one of the documents fail to compile', async done => {
      ...
      jest.mock('request');
      rp.get = jest.fn()
        .mockImplementationOnce((options, cb) => cb(null, {body:'chunk'}))
    ...
    test('Should create the release with all the compiled files', async done => {
      ...
      jest.mock('request');
      rp.get = jest.fn()
        .mockImplementationOnce((options, cb) => cb(null, {body:'chunk1'}))
        .mockImplementationOnce((options, cb) => cb(null, {body:'chunk2'}))
