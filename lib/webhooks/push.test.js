  ...
  describe('Retrieve file to compile', () => {
    test('Should fetch files name to compile from the config', async done => {
      ...
      const context = {
        ...
        config: jest.fn(() => Promise.reject(new Error('ee')))
      }
      try {
        await push(context)
      } catch (e) {
    ...
    test('Should create an issue if the file is not exist', async done => {
      ...
      try {
        ...
        expect(robot.utils.issues.create.mock.calls[0][2]).toEqual('user_foo')
        expect(robot.utils.issues.create.mock.calls[0][3]).toEqual(['foo/repo'])
        expect(robot.utils.issues.create.mock.calls[0][4]).toEqual({files: 'resume-1.tex, resume-2.tex'})
        done()
      }
    })
    ...
    test('Should create an issue when it fail to compile the files', async done => {
      ...
      rp.get = jest.fn()
        .mockImplementationOnce((options, cb) => {
          cb(null, { statusCode: 400 }, 'first call')
          _event.emit('close')
          return { pipe }
        })
      ...
      try {
        ...
        expect(robot.utils.notify.mock.calls[0][0]).toEqual('I just generate successfully 0 resume (0/1)')
        done()
      } catch (e) {
        done(e)
      }
    ...
    test('Should create an issue when the bot can\'t read the compiles file', async done => {
      const context = {
        payload: {
          ref: 'refs/heads/master',
          repository: {
      ...
      fs.readFile = jest.fn()
        .mockImplementation((path, cb) => cb(new Error('error : err')))

      rp.get = jest.fn()
        .mockImplementationOnce((options, cb) => {
          cb(null, { statusCode: 200 }, 'first call')
          _event.emit('close')
          return { pipe }
        })
      ...
      try {
        ...
        expect(robot.utils.issues.create.mock.calls[0][2]).toEqual('user_foo')
        expect(robot.utils.issues.create.mock.calls[0][3]).toEqual(['foo/repo'])
        expect(robot.utils.issues.create.mock.calls[0][4]).toEqual({file: 'resume-1.tex', error: new Error('error : err')})
        expect(robot.utils.notify).toHaveBeenCalledTimes(2)
        expect(robot.utils.notify.mock.calls[0][0]).toEqual('Error - fs.readFile - error : err')
        expect(robot.utils.notify.mock.calls[1][0]).toEqual('I just generate successfully 0 resume (0/1)')
        done()
      } catch (e) {
        done(e)
      }
    ...
    test('Should create an issue when it all the documents fail to compile', async done => {
      ...
      try {
        ...
        expect(robot.utils.notify).toHaveBeenCalledTimes(1)
        expect(robot.utils.notify.mock.calls[0][0]).toEqual('I just generate successfully 0 resume (0/2)')
        done()
      } catch (e) {
        done(e)
      }
    ...
    test('Should create an issue when it one of the documents fail to compile', async done => {
      ...
      fs.readFile = jest.fn()
        .mockImplementation((path, cb) => cb(null, 'chunk'))

      fs.unlinkSync = jest.fn()

      rp.get = jest.fn()
        .mockImplementationOnce((options, cb) => {
          cb(null, { statusCode: 200 }, 'chunk')
          _event.emit('close')
          return { pipe }
        })
        .mockImplementationOnce((options, cb) => {
      ...
      try {
        ...
        expect(robot.utils.notify).toHaveBeenCalledTimes(1)
        expect(robot.utils.notify.mock.calls[0][0]).toEqual('I just generate successfully 1 resume (1/2)')
        done()
      } catch (e) {
        done(e)
      }
    ...
    test('Should create the release with all the compiled files', async done => {
      ...
      fs.readFile = jest.fn()
        .mockImplementationOnce((path, cb) => cb(null, 'chunk1'))
        .mockImplementationOnce((path, cb) => cb(null, 'chunk2'))

      fs.unlinkSync = jest.fn()

      rp.get = jest.fn()
        .mockImplementationOnce((options, cb) => {
          cb(null, { statusCode: 200 }, 'chunk1')
          _event1.emit('close')
          return { pipe }
        })
        .mockImplementationOnce((options, cb) => {
          cb(null, { statusCode: 200 }, 'chunk2')
          _event2.emit('close')
          return { pipe }
        })
      ...
      try {
        ...
        expect(robot.utils.notify).toHaveBeenCalledTimes(1)
        expect(robot.utils.notify.mock.calls[0][0]).toEqual('I just generate successfully 2 resume (2/2)')
        done()
      } catch (e) {
        done(e)
      }
