  ...
  return async context => {
    ...
    const requests = treeFiles.map(file => {
      ...
      return new Promise((resolve, reject) => {
        ...
        rp.get(options, (err, res) => resolve(err || {file, data : res}));
      });
    });
   
    const results = await Promise.all(requests)

    const fails = results.filter(result => (result.name || '').match(/error$/i))
    fails.forEach(result => {
      robot.utils.issues.create(github, 'issue_fail_compile', payload.sender.login, [payload.repository.full_name], {error: result});
    });

    const success = results.filter(result => !(result.name || '').match(/error$/i))
    success.forEach(result => {
      robot.utils.release.create(github, payload.sender.login, [payload.repository.full_name], result);
    });
